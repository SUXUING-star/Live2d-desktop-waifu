...
ICubismModelSetting* LAppModel::GetModelSetting() const
{
    return _modelSetting;
}


float LAppModel::GetModelLeft() const
{
    if (!_model) return 0.0f;
    
    // 获取底层的 csmModel* 指针！这才是关键！
    csmModel* coreModel = _model->GetModel();
    if (!coreModel) return 0.0f;

    const csmVector2** vertexPositions = csmGetDrawableVertexPositions(coreModel);
    const int* vertexCounts = csmGetDrawableVertexCounts(coreModel);
    int drawableCount = csmGetDrawableCount(coreModel);
    ...
    for (int i = 0; i < drawableCount; ++i)
    {
        ...
        {
            if (vertexPositions[i][j].X < minX)
            {
                minX = vertexPositions[i][j].X;
            }
        }
    }
    return minX;
...
float LAppModel::GetModelRight() const
{
    if (!_model) return 0.0f;
    ...
    for (int i = 0; i < drawableCount; ++i)
    {
        for (int j = 0; j < vertexCounts[i]; ++j)
        {
            if (vertexPositions[i][j].X > maxX)
            {
                maxX = vertexPositions[i][j].X;
            }
        }
    }
    return maxX;
...
float LAppModel::GetModelTop() const
{
    if (!_model) return 0.0f;
    ...
    for (int i = 0; i < drawableCount; ++i)
    {
        for (int j = 0; j < vertexCounts[i]; ++j)
        {
            if (vertexPositions[i][j].Y > maxY)
            {
                maxY = vertexPositions[i][j].Y;
            }
        }
    }
    return maxY;
...
float LAppModel::GetModelBottom() const
{
    if (!_model) return 0.0f;
    ...
    for (int i = 0; i < drawableCount; ++i)
    {
        for (int j = 0; j < vertexCounts[i]; ++j)
        {
            if (vertexPositions[i][j].Y < minY)
            {
                minY = vertexPositions[i][j].Y;
            }
        }
    }
    return minY;
